...
class WeatherRepository extends BaseRepository {
  ...
  Future<Either<String?, WeatherCardModel>> getWeather(String city) async {
    return handleNetworkCall<WeatherCardModel, WeatherCardModel>(
        apiCall: _weatherDataSource.getWeather(city),
        onSuccess: (data) => data);
  }

  Future<Either<String?, WeatherCardModel>> getWeatherFromLatLong(
      {required double lat, required double long}) {
    return handleNetworkCall(
        apiCall: _weatherDataSource.getWeatherFromLatLong(lat: lat, long: long),
        onSuccess: (data) => data);
  }
